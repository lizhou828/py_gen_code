...
class Logger(object):
    ...
    def outMsg(self, msg, enable=True):
        ...
        if enable:
            ...
            # print("%s >>> [class:%s][method:%s][lineNO:%s] [Msg] %s" % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self.__obj.__name__, sys._getframe().f_code.co_name, sys._getframe().f_lineno, msg))
        pass