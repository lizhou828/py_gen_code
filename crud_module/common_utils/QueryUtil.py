...
class QueryUtil(object):
    ...
    def queryByPrimaryKey(primaryKeyDict, value, columnList):
        if (not primaryKeyDict) or len(primaryKeyDict) == 0 or (not isinstance(primaryKeyDict, dict)):
            print("primaryKeyDict is not valid!!!!")
            return
        """
        拼接主键查询
        """
        sql = 'SELECT %s FROM `%s` WHERE `%s`="%s"' % (
            QueryUtil.queryColumns(columnList), primaryKeyDict["tableName"], primaryKeyDict["primaryKey"], str(value))
        return sql
    ...
    @staticmethod
    def queryCount(tableName, objDict):
        """
        拼接查询记录数
        """
        if (not objDict) or (len(objDict) == 0) or (not isinstance(objDict, dict)):
            ...
            sql = "SELECT COUNT(*) FROM  `%s` where " % (tableName)
            i = 0
    ...
    def queryAllByPage(tableName, columnList, page=None):
        ...
        if not page:
            page = Page()
        return 'SELECT %s FROM %s LIMIT %d,%d' % (
            QueryUtil.queryColumns(columnList), tableName, page.get_begin(), page.get_limit())
