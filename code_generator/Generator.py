...
class PythonGenarator(object):
    ...
    def writeImport(self,file ,importList = None):
        ...
        self.__content += "from com.diyun.python.crud_module.common_utils.DateEncoder import DateEncoder\r\n"
        pass
    ...
    def writeStr(self, file, fileName, tableDictString):
        ...
        self.__content += "\n\tdef __str__(self):\n\t\t%sDict = %s\r\t\treturn json.dumps(%sDict , cls=DateEncoder)\n" % (fileName, tableDictString, fileName)
        pass
...
class DaoGenarator(object):
    ...
    def run(self):
        for filePath in self.__configDict["daoPathList"]:
            if not os.path.exists(filePath):
                os.makedirs(os.path.dirname(filePath), exist_ok=True)
            ...
            # 实体类的类名
            domainName = ClassName.replace("dao","")
            domainName = ClassName.replace("Dao","")

            # 实体类的文件名
            domainFileName = domainName.lower()

            # 打开新建文件
            file = open(filePath, "w", encoding="utf-8")
            daoStr = "# -*- coding:utf-8 -*-\n" +\
                     "from com.diyun.python.crud_module.dao.BaseDao import BaseDao\n" + \
                             "from com.diyun.python.crud_module.domain.%s import %s\n\r\r" + \
                             "class %s(BaseDao):\n"+\
                             "\tdef __init__(self):\n"+\
                             "\t\tsuper().__init__(%s)\n"+\
                             "\t\tpass\n"

            self.__content = daoStr % (domainFileName,domainName,ClassName,domainName)
